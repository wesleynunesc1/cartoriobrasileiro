        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {LIFE_MOMENTS.slice(0, 4).map((moment) => (
            <div
              ...
              className="rounded-2xl bg-white border border-[#0D1B3D]/10 hover:border-[#C6A166] transition-all duration-300 hover:shadow-lg cursor-pointer flex flex-col justify-between group overflow-hidden"
            >
              <div>
                {/* Imagem Superior */}
                {moment.imageUrl && (
                  <div className="relative h-40 w-full overflow-hidden">
                    ...
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-[#0D1B3D] via-[#0D1B3D]/40 to-transparent opacity-95" />
                    
                    <div className="absolute bottom-3 left-4 right-4 flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-lg bg-[#0D1B3D]/80 backdrop-blur-md text-[#C6A166] flex items-center justify-center border border-white/20">
                        <DynamicIcon name={moment.iconName} className="w-4 h-4" />
                      </div>
                      <span className="text-[11px] font-bold text-white leading-tight">
                        {moment.imageBadge || moment.subtitle}
                      </span>
                    </div>
                  </div>
                )}
                ...
                <div className="p-6 pb-0">
                  <h3 className="font-serif text-lg lg:text-xl font-semibold text-[#0D1B3D] mb-3">
                    {moment.title}
                  </h3>

                  <p className="text-[13px] text-[#0D1B3D]/70 leading-relaxed font-sans mb-6">
                    {moment.description}
                  </p>

                  {/* Highlights */}
                  <div>
                    <span className="text-[10px] font-bold tracking-[0.15em] text-[#A68249] uppercase block mb-3">
                      POSSIBILIDADES:
                    </span>
                    <div className="space-y-2.5">
                      {moment.highlights.slice(0, 3).map((item, idx) => (
                        <div key={idx} className="flex items-start gap-2.5 text-[12px] text-[#0D1B3D]/80 leading-tight">
                          <CheckCircle className="w-3.5 h-3.5 text-[#2F6B57] flex-shrink-0 mt-0.5" />
                          <span>{item}</span>
                        </div>
                      ))}
                    </div>
              ...
              <div className="mx-6 mb-6 mt-6 pt-4 border-t border-[#0D1B3D]/10 flex items-center justify-between text-xs font-semibold text-[#A68249] group-hover:text-[#C6A166] transition-colors">
